...
class MercadoSpider(CrawlSpider):
    ...
    rules = (
        ...
        Rule(LinkExtractor(allow=r'.*/_Desde_\d+$'),follow=True),#下一页
        Rule(LinkExtractor(allow=r'.*/MLM(\d+|-\d+)',deny=(r'.*/jms.*',r'.*noindex.*',r'.*auth.*',r'.*=.*',r'.*product_trigger_id=MLM+\d+',r'.*/seller-info$',
    ...
    def parse (self,response):
        ...
        print(like_count)
        #打印店铺
        serller = response.xpath('//a[@class="ui-pdp-action-modal__link"]/span[@class="ui-pdp-color--BLUE"]/text()').get()
